...
function capitalA(s) {
    ...
    if (/^[a-zA-Z\s]+$/g.test(s) && s[s.length - 1] === 'A') {
...
function imagesSearcher(text) {
    let arr = [];
    // Add your logic.
    let extentionImage = ["png", "jpg", "jpeg"];

    let imageIn = text.split(/\s+/);

    imageIn.forEach((img) => {
        if (extentionImage.some((extentionImage) => img.endsWith(extentionImage))) {
            arr.push(img);
        }
    });
